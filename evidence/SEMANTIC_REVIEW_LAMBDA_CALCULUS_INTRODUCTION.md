...
Units: OLP-0341--OLP-0355

Scope: 96 aligned source/target blocks across the Lambda Calculus part driver and the complete Introduction chapter: overview, introductory syntax and reduction, Church--Rosser, currying, lambda definability, both computability directions, the basic primitive-recursive lambda definitions, and closure under composition, primitive recursion, and minimization, with fixed-point combinators supplying the last construction. Of these, 77 blocks contain reviewed Bengali translation and 19 are unchanged structural or formal blocks.

The Bengali text was read against the frozen English blocks in source order and reverse-paraphrased at the level of lambda notation, abstraction and application, inductive term formation, alpha-equivalence, capture-avoiding substitution, beta-contraction and reduction, normal forms, the Church--Rosser joining property, currying, Church numerals, lambda definability, the two computability implications, the normal-form proof plan, initial primitive-recursive functions, composition, ordered-pair state iteration for primitive recursion, Curry and Turing fixed-point combinators, and fixed-point search for minimization. Every source/target pair passes block-count, audited-mathematics, control-sequence, environment, semantic-token, and Unicode NFC checks. This is a source and semantic review by the primary translation model; no independent human review, TeX-engine result, or PDF result is claimed. The part and chapter editorial notes about provenance, redundancy, and needed revision are retained in Bengali.
...
| OLP-0353 | Primitive recursion | 11 | 11 | Parameter absorption, ordered-pair selector, state transition, and iterator proof |
| OLP-0354 | Fixed-point combinators | 2 | 2 | Diagonal self-application and the distinct Curry and Turing reduction claims |
| OLP-0355 | Minimization | 6 | 6 | Fixed-point search from an index, zero test, partiality, and absence of normal form |

## Source review outcome

Eight source-level defects were corrected with adjacent Bengali notes and exact validator transformations.
...
5. **BN-SRC-271 (OLP-0353):** the primitive-recursion setup inconsistently used `G,H` and `G',H'` for the supplied definers, called the desired term `H` instead of `F`, and used the wrapper names before defining them. The target uses `G',H'` for the supplied definers, `F` for the constructed term, and reserves `G,H` for the parameter-absorbing wrappers.
6. **BN-SRC-272 (OLP-0353):** the defining recurrence passed `z` as the first argument of `h`, although `x` is the recursion index and the later numeral equation correctly passes `n`. The target restores `h(x,f(x,\vec z),\vec z)`.
7. **BN-SRC-273 (OLP-0353):** the parameter-absorbing step applied `v=F(n)` to both `u` and `\vec z`, repeating the recursion index. The target writes `v(\vec z)`, which recovers the preceding value `F(n,\vec z)`.
8. **BN-SRC-274 (OLP-0355):** the minimization lemma assumes that `f` is lambda definable, but its proof unnecessarily called `f` primitive recursive. The target obtains the defining term `F` directly from the stated lambda-definability premise.

OLP-0344 contains the ordinary prose typo `From example`; the Bengali rendering uses the intended `যেমন` (“for example”). This does not alter a formal object or technical claim and is therefore not entered as a source-correction record. The remaining terms, reductions, theorem claims, and proof plans in OLP-0341--OLP-0355 are internally consistent.
...
15. **Primitive recursion:** the corrected opening first absorbs the extra parameter vector into a base term `G` and a binary step term `H`, leaving a unary recursion on Church numerals. The selector `D(M,N)` returns `M` at Church zero and `N` at Church one, so it represents an ordered pair with two projections. The transition `T` advances the numeral and computes the next value; iterating `T` with a Church numeral and taking the second projection gives the required recursive value. BN-SRC-271 through BN-SRC-273 preserve the same term roles and preceding-value invariant from the parameterized equations through this state construction.
16. **Fixed-point combinators:** diagonal self-application makes `k=ll` reduce to `gk`. Curry’s `Y` sends both `Yg` and `g(Yg)` to a common reduct, yielding beta-equivalence. Turing’s displayed variant satisfies the stronger directed reduction `Yg \red g(Yg)`. The translation keeps these claims distinct rather than treating the two formulas as interchangeable.
17. **Minimization:** for fixed `x`, `h_x(n)` tests `f(x,n)` and either returns `n` at zero or continues from `n+1`; `Y` solves this recursive fixed-point equation. The selector’s behavior on zero versus a positive Church numeral implements the branch, while an undefined intermediate computation prevents a normal form, matching unbounded minimization for partial functions. Abstracting the free `x` and starting at zero yields the desired term `G`. BN-SRC-274 ensures that this proof uses only the lemma’s lambda-definability premise.
...
BN-IN-T291 records currying and function-valued functions; BN-IN-T292 records Church numerals and iteration; BN-IN-T293 records the lambda-definability/computability equivalence, Church's thesis, and reduction-tree search; BN-IN-T294 records the Kleene-normal-form proof route, conventional arity notation, and the three closure operations; BN-IN-T295 records the basic primitive-recursive lambda definitions and composition closure; BN-IN-T296 records the ordered-pair selector, constant function, projections, and primitive-recursion closure construction; and BN-IN-T297 records the fixed-point search proving closure under minimization. Existing T017, T057, T099, T150, T154, T157, T172, T173, T177, T198, T242, and T289 continue to govern ordered pairs, successor, substitution, primitive recursion, unbounded search and minimization, notation, normal forms, partial computability, fixed-point combinators, free-for-substitution vocabulary, and beta-equivalence.
...
The per-unit checker recognizes all 354 translated target units and passes the fifteen reviewed units individually. After state export, the cumulative index contains 4,045 aligned blocks: 3,417 translated and semantically reviewed, and 628 unchanged structural or formal blocks. OLP-0353--OLP-0355 contributes 19 translated blocks. The cumulative 299-unit semantic reader remains the current published reader tranche; reader rebuilding is deferred while source translation continues. The next source gate is the 354-unit public checkpoint with anonymous immutable-archive readback.
