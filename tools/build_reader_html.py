...
for name in ('union','intersection','difference'):
    body=body.replace('\\olasset{assets/diagrams/'+name+'.tikz}','DIAGRAM'+name.upper()+'TOKEN')
...
result=subprocess.run(['pandoc',str(source),'-f','latex','-t','html5','--standalone','--mathml','--toc','--metadata','lang=bn-IN','--metadata','title=ওপেন লজিক: সেট ও সম্পর্ক','-o',str(out)],cwd=build,capture_output=True,text=True,encoding='utf-8')
...
assert len(soup.find_all('h2'))==15
assert len(soup.select('div.defn,div.ex,div.prop,div.thm,div.prob'))==env_count
assert len(soup.select('img.diagram'))==6
assert 'DIAGRAM' not in soup.get_text()
out.write_text(str(soup),encoding='utf-8')
receipt={'html_bytes':out.stat().st_size,'html_sha256':hashlib.sha256(out.read_bytes()).hexdigest(),'input_tex_sha256':hashlib.sha256((build/'openlogic-bn-reader.tex').read_bytes()).hexdigest(),'pdf_sha256':hashlib.sha256((build/'openlogic-bn-reader.pdf').read_bytes()).hexdigest(),'sections':15,'numbered_environments':env_count,'mathml_expressions':len(soup.find_all('math')),'resolved_source_references':ref_count,'broken_internal_links':broken,'figures':figure_receipts,'offline_fonts':True,'pandoc_warnings':0,'visual_status':'pending browser inspection'}
