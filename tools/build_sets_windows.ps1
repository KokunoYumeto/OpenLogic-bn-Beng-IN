param([ValidateSet('sets','reader')][string]$Edition='sets')
$ErrorActionPreference = 'Stop'
$taskRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
$taskBuild = Join-Path $taskRoot ('build\' + $Edition)
$taskBase = 'openlogic-bn-' + $Edition
...
$taskRecord = @{edition=$Edition;status='starting';mutex='Global\InterlanguageTeXSlotV1';acquisition_timeout_ms=120000;input_sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $taskResolvedBuild ($taskBase + '.tex'))).Hash.ToLowerInvariant();started_utc=[DateTime]::UtcNow.ToString('o');acquired=$false}
...
try {
    ...
    try {
        for ($taskPass = 1; $taskPass -le 4; $taskPass++) {
            ...
            try {
                ...
                while ([BengaliTexJob]::Active($taskJob) -gt 0) { if ([DateTime]::UtcNow -gt $taskDeadline) { [void][BengaliTexJob]::TerminateJobObject($taskJob,124); throw 'Captured TeX descendants exceeded 90-second bound' }; [Threading.Thread]::Sleep(100) }
        ...
        $taskReceipt = @{mutex='Global\InterlanguageTeXSlotV1';acquisition_timeout_ms=120000;acquired=$true;abandoned_recovery=$taskAbandoned;passes=$taskPassResults;stable_pdf_between_passes=($taskPassResults[-1].pdf_sha256 -eq $taskPassResults[-2].pdf_sha256);log_findings=$taskFindings;tree_policy='Captured Windows job; parent and descendants end before pass completion. Shell escape and installer disabled; mutex spans up to four convergence passes and immediate log checks.'}
