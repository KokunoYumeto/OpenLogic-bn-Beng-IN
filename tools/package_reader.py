...
archive=dist/'openlogic-bn-Beng-IN-sets-relations-sources.zip'
...
for suffix in ['pdf','html']:
    target=dist/f'openlogic-bn-Beng-IN-sets-relations.{suffix}'
...
(dist/'SHA256SUMS-sets-relations.txt').write_text(''.join(r['sha256']+'  '+r['filename']+'\n' for r in rows),encoding='ascii')
