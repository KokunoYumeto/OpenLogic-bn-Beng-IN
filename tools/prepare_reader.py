...
preamble=preamble.replace('pdftitle={ওপেন লজিক: সেট}','pdftitle={ওপেন লজিক: সেট ও সম্পর্ক}')
preamble=preamble.replace('Sets chapter; full edition in progress','Sets and Relations; full edition in progress')
preamble=preamble.replace('{\\LARGE সেট\\par}','{\\LARGE সেট ও সম্পর্ক\\par}')
preamble=preamble.replace('এই অধ্যায়টি','এই অধ্যায়গুলি').replace('এই অংশে উৎসের OLP-0004 থেকে OLP-0010 এবং OLP-0721 অন্তর্ভুক্ত।','এই অংশে উৎসের OLP-0004 থেকে OLP-0019 এবং OLP-0721-এর পাঠ অন্তর্ভুক্ত। বিকল্প অধ্যায়-চালক OLP-0719 সম্পাদনাযোগ্য উৎসে আছে; সেটি একই অধ্যায় পুনরাবৃত্তি না করে দার্শনিক অংশ বাদ দেওয়ার বিন্যাসটি দেয়। পূর্ণ 722টি উৎস ফাইলের মধ্যে 18টির অনুবাদ এই সংস্করণে আছে।')
...
parts=[preamble]
groups={'sets':['basics','subsets','important-sets','unions-and-intersections','pairs-and-products','russells-paradox','proofs-about-sets'],'relations':['relations-as-sets','reflections','special-properties','equivalence-relations','orders','graphs','trees','operations']}
notes={
...
for group,names in groups.items():
    ...
    for name in names:
        ...
        words={'element':'উপাদান','formula':'সূত্র','derivation':'নিষ্পাদন'}
        text=re.sub(r'!!(\^)?(a)?\{(element|formula|derivation)\}(s)?',lambda m:('একটি ' if m[2] else '')+words[m[3]],text)
        assert '!!' not in text
        if name in notes:
...
print('Prepared cumulative Sets and Relations reader: 18 translated source units, alternate driver integrated without duplication.')
